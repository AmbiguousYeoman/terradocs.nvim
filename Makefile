...
# Test runner - uses plenary's test_harness for proper exit handling
test: deps
	@echo "Running all tests..."
	nvim --headless --noplugin -u tests/minimal_init.lua \
		-c "lua require('plenary.test_harness').test_directory('$(TESTS_DIR)', { minimal_init = 'tests/minimal_init.lua' })"
...
test-init: deps
	...
	nvim --headless --noplugin -u tests/minimal_init.lua \
		-c "lua require('plenary.busted').run('tests/terradocs/init_spec.lua')"

# Run only ts_helper tests
test-ts-helper: deps
	@echo "Running ts_helper.lua tests..."
	nvim --headless --noplugin -u tests/minimal_init.lua \
		-c "lua require('plenary.busted').run('tests/terradocs/ts_helper_spec.lua')"
